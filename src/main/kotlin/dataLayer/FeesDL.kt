...
class FeesDL {
    ...
    fun insertNewStu(id:Int)
    {
        try {
            ...
            while (particularList?.next() == true) {
                val query2 =
                    "UPDATE StudentFeesTable set `${particularList.getString("Particulars")}`='Unpaid' where studentId=$id"
    ...
    fun getPaidStatus(id:Int,particular: String):ResultSet?=
        try
        {
            val query="SELECT `$particular` from StudentFeesTable where studentId=$id"
            val st=con?.createStatement()
            st?.executeQuery(query)
        }
        catch (e:Exception)
